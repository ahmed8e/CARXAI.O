...
export default function AIMechanic() {
  ...
  const fetchUsageCount = async (): Promise<boolean> => {
    ...
    try {
      ...
      console.error('[Car Safety] Usage check failed:', err)
    }
    return false
  }
  ...
  const incrementUsage = async (type: 'chat' | 'report' | 'image') => {
    ...
    try {
      ...
      console.error('[Car Safety] Failed to increment usage:', err)
    }
  }
  ...
  const fetchActiveVehicle = async () => {
    ...
    try {
      ...
      if (vehicles && vehicles.length > 0) {
        ...
        console.log('[Car Safety AI] Active vehicle loaded:', defaultVehicle.make, defaultVehicle.model);
        setActiveVehicle(defaultVehicle as Vehicle)
      } else {
        console.log('[Car Safety AI] No active vehicle found for user.');
  ...
  const sendMessage = async (content: string, imageUrl?: string, chipLabel?: string, customContext?: any) => {
    ...
    try {
      ...
      console.log('[Car Safety AI] Vehicle context for analysis:', vehicleContext);
...
(Note: Use this history to spot recurring patterns or unresolved issues.)`
      ...
      const performAnalysis = async (isRetry = false, customContext?: any) => {
        console.log(`[Car Safety AI] Starting analysis (isRetry: ${isRetry})`);
      ...
      let accumulatedJSON = await performAnalysis(false, customContext);
        console.log('[Car Safety AI] Raw accumulated JSON from OpenAI:', accumulatedJSON);
        ...
        try {
          ...
          console.log('[Car Safety AI] RAW API Payload:', accumulatedJSON)
          console.log('[Car Safety AI] Parse result:', parseResult)

          if (!parseResult.success) {
            console.warn('[Car Safety AI] First parse failed, retrying once...', parseResult.reason)
            accumulatedJSON = await performAnalysis(true, customContext)
            console.log('[Car Safety AI] RETRY RAW API Payload:', accumulatedJSON)

            parseResult = extractStructuredPayload(accumulatedJSON)
            console.log('[Car Safety AI] Retry parse result:', parseResult)
          }
          ...
          console.error('[Car Safety AI] Final Logic Catch Triggered:', (err as any)?.message || err)
          console.log('[Car Safety AI] Raw accumulatedJSON at time of failure:', accumulatedJSON)
        ...
        if (issueData && !issueData.needs_followup) {
          ...
          if (insertedChat && insertedChat.id) {
            ...
            console.error('[Car Safety AI] Failed to save chat to DB:', chatError)
          }
  ...
  const handleFileUpload = async (file: File) => {
      ...
      try {
        ...
        console.error('[Car Safety AI] Image upload failed:', err)
