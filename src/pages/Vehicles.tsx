    ...
    <div className="p-6 lg:p-10 max-w-6xl mx-auto min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[11px] font-black uppercase tracking-[0.2em] text-navy/70">{t('vehicles.manager')}</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-display font-black text-on-surface italic tracking-tight">{t('vehicles.title')}</h1>
          <p className="text-muted/70 font-medium mt-2">{t('vehicles.track_desc')}</p>
        </div>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
        >
          <Plus className="w-5 h-5" /> {t('vehicles.add_button')}
        </motion.button>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          ...
          <p className="text-xs font-black uppercase tracking-widest text-muted">{t('vehicles.loading_garage')}</p>
        </div>
      ) : vehicles.length === 0 ? (
        <div className="bg-surface dark:bg-surface-low border-2 border-dashed border-overlay rounded-[32px] p-20 text-center">
          <div className="w-20 h-20 rounded-full bg-surface-low dark:bg-surface-high/40 border border-overlay hover:bg-surface-high transition-all text-start font-bold text-on-surface/80 flex items-center justify-between group mx-auto">
            <Car className="w-10 h-10 text-muted mx-auto" />
          </div>
          <h3 className="text-2xl font-display font-bold text-on-surface mb-2">{t('vehicles.add_first')}</h3>
          <p className="text-muted max-w-md mx-auto mb-8 font-medium">
            {t('vehicles.track_desc')}
          </p>
          <button
            onClick={() => handleOpenModal()}
            ...
            >
              <div className="flex items-start justify-between mb-6">
                ...
                {v.is_default && (
                  <div className="px-3 py-1 rounded-full bg-navy text-[10px] font-black uppercase tracking-widest text-white flex items-center gap-1.5 shadow-lg shadow-navy/20">
                    <ShieldCheck className="w-3 h-3" /> {t('vehicles.active_badge')}
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-on-surface/80 truncate">{v.mileage ? `${v.mileage.toLocaleString(i18n.language)} ${t('maintenance.mileage_unit')}` : 'N/A'}</span>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 pt-6 border-t border-overlay">
                {!v.is_default ? (
                  ...
                  <div className="flex-1 text-[10px] font-black uppercase tracking-widest text-navy text-center flex items-center justify-center gap-1 font-display">
                    <CheckCircle2 className="w-3 h-3" /> {t('vehicles.default')}
                  </div>
                )}
                <button
