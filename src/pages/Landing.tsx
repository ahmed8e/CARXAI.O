...
export default function Landing() {
  ...
  return (
    <div className="relative min-h-screen bg-surface dark:bg-slate-900 text-on-surface selection:bg-navy/10 transition-colors duration-300">
      ...
      <div className="relative z-10">
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: -20 }}
              className="fixed inset-0 z-40 bg-surface/98 dark:bg-slate-900/98 backdrop-blur-2xl flex flex-col items-center justify-center gap-8 p-8"
            >
              <button 
                onClick={() => setMobileMenuOpen(false)}
                className="absolute top-8 right-8 w-12 h-12 rounded-full bg-surface-low dark:bg-slate-800 flex items-center justify-center text-muted"
              >
                <X className="w-6 h-6" />
              </button>
              ...
              {user ? (
                <div className="flex flex-col items-center gap-4 w-full px-10">
                  <div className="w-24 h-24 rounded-full bg-gradient-to-br from-navy to-navy/80 flex items-center justify-center text-white text-4xl font-display font-bold shadow-2xl shadow-navy/20 ring-4 ring-surface-low dark:ring-slate-800 border border-navy/10 mb-4 relative overflow-hidden">
                  ...
                  <div className="w-full space-y-3">
                    ...
                    <Link to="/my-account" className="flex items-center justify-center gap-3 w-full py-4 rounded-2xl border border-overlay bg-surface dark:bg-slate-800 text-on-surface font-bold" onClick={() => setMobileMenuOpen(false)}>
        ...
        <section className="relative min-h-[90vh] flex items-center px-6 pt-[calc(8rem_+_env(safe-area-inset-top))] pb-20 overflow-hidden">
          <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center relative z-10">
            ...
            <div className="text-left">
              ...
              >
                {[
                  ...
                  <div key={i} className="flex flex-row sm:flex-col items-center sm:items-start gap-4 sm:gap-3 p-3.5 sm:p-5 rounded-[20px] sm:rounded-[32px] bg-surface dark:bg-slate-800 border border-overlay shadow-sm hover:border-navy/20 hover:shadow-xl transition-all group">
              ...
              >
                ...
                <button 
                  ...
                  className="w-full sm:w-auto px-10 py-5 rounded-2xl border border-overlay bg-surface dark:bg-slate-800 text-on-surface font-bold hover:bg-surface-low transition-all"
                >
                  See How It Works
                </button>
              </motion.div>
            ...
            >
              <div className="relative w-full aspect-square md:aspect-video lg:aspect-[4/5] max-w-xl rounded-[40px] bg-surface-low dark:bg-slate-800 border border-overlay shadow-2xl overflow-hidden group-hover:scale-[1.02] transition-transform duration-700">
                ...
                <motion.div 
                  className="absolute top-4 left-4 sm:top-8 sm:left-8 p-4 sm:p-5 rounded-[24px] bg-surface/95 dark:bg-slate-800/95 backdrop-blur-2xl border border-overlay shadow-xl flex items-center gap-3 sm:gap-4 z-20"
                  animate={{ y: [0, -8, 0] }}
                ...
                <motion.div 
                  className="absolute bottom-4 right-4 sm:bottom-8 sm:right-8 p-4 sm:p-5 rounded-[24px] bg-surface/95 dark:bg-slate-800/95 backdrop-blur-2xl border border-overlay shadow-xl flex items-center gap-3 sm:gap-4 z-20"
                  animate={{ y: [0, 8, 0] }}
          ...
          <div className="max-w-4xl mx-auto text-center flex flex-col items-center">
            ...
            <div className="grid md:grid-cols-3 gap-5 w-full">
              {[
                ...
                <div key={i} className="group p-6 rounded-[32px] bg-surface dark:bg-slate-800 border border-overlay shadow-sm hover:border-navy/20 hover:shadow-xl transition-all text-left flex flex-col items-start gap-4 h-full">
        ...
        <section id="how-it-works" className="pt-6 pb-12 md:pt-10 md:pb-20 px-0 overflow-hidden">
          ...
          <div className="flex gap-4 overflow-x-auto snap-x scrollbar-hide px-6 md:grid md:grid-cols-4 md:max-w-6xl md:mx-auto md:px-0 md:overflow-visible">
            {steps.map((step, i) => (
              <div 
                key={i} 
                className="flex-shrink-0 w-[240px] md:w-auto snap-center p-8 rounded-[32px] bg-surface dark:bg-slate-800 border border-overlay shadow-sm relative group hover:border-navy/20 hover:shadow-xl transition-all overflow-hidden"
              >
          ...
          <div className="relative">
            ...
            >
              {[...features, ...features, ...features].map((feature, i) => (
                <div 
                  key={i} 
                  className="flex-shrink-0 w-[280px] p-10 rounded-[32px] bg-surface-low dark:bg-slate-800 border border-overlay shadow-sm hover:bg-surface hover:border-navy/20 hover:shadow-xl transition-all group"
                >
                  <div className="w-14 h-14 rounded-2xl bg-surface dark:bg-slate-900 flex items-center justify-center mb-8 border border-overlay group-hover:bg-navy group-hover:text-white transition-all shadow-sm">
          ...
          <div className="relative group/marquee">
            ...
            >
              {[...reviews, ...reviews, ...reviews].map((rev, i) => (
                <div 
                  key={i} 
                  className="flex-shrink-0 w-[320px] md:w-[400px] p-10 rounded-[32px] bg-surface dark:bg-slate-800 border border-overlay shadow-sm relative group transition-all duration-500 overflow-hidden hover:shadow-xl hover:border-navy/20"
                >
                  <div className="relative z-10 flex flex-col h-full">
                    {/* Stars */}
        ...
        <section id="pricing" className="py-12 md:py-20 px-6">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {plans.map((plan, i) => (
                <div key={i} className={`p-10 rounded-[32px] border transition-all ${plan.popular ? 'bg-surface dark:bg-slate-800 border-navy shadow-2xl scale-105 z-10' : 'bg-surface-low dark:bg-slate-800 border-overlay shadow-sm hover:shadow-xl'} relative group`}>
                  {plan.popular && (
                  ...
                  <button 
                    ...
                    className={`w-full py-5 rounded-2xl font-black transition-all text-base ${plan.popular ? 'bg-navy text-white shadow-xl hover:-translate-y-1' : 'bg-surface dark:bg-slate-700 border border-overlay text-on-surface hover:bg-surface-high'}`}
                  >
        ...
        {/* Footer */}
        <footer className="py-20 px-6 bg-surface-low dark:bg-slate-950 border-t border-overlay">
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row gap-16 md:gap-24 mb-16">
              {/* Brand Col */}
              <div className="flex flex-col items-start gap-6 max-w-sm">
                <div className="flex items-center gap-3 group">
                  <div className="w-12 h-12 rounded-xl bg-surface dark:bg-slate-900 border border-overlay flex items-center justify-center shadow-sm group-hover:shadow-md group-hover:border-navy/30 transition-all">
                    <Zap className="w-6 h-6 text-navy" />
                  </div>
                  <span className="font-display font-bold text-2xl tracking-tighter text-on-surface">
                    Carxai
                  </span>
                </div>
              ...
              <div className="flex-1 grid grid-cols-2 md:grid-cols-3 gap-10">
                ...
                <div className="flex flex-col gap-5 col-span-2 md:col-span-1 border-t border-overlay pt-8 md:border-0 md:pt-0">
                  ...
                  <div className="flex gap-3">
                    <a href="https://twitter.com" target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full bg-surface dark:bg-slate-900 border border-overlay flex items-center justify-center text-muted hover:text-navy hover:border-navy/30 hover:shadow-md transition-all">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"/></svg>
                    </a>
                    <a href="https://linkedin.com" target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full bg-surface dark:bg-slate-900 border border-overlay flex items-center justify-center text-muted hover:text-navy hover:border-navy/30 hover:shadow-md transition-all">
