    ...
    <AnimatePresence>
      ...
      >
        <div className="relative overflow-hidden rounded-[32px] bg-white border border-slate-100 shadow-[0_30px_70px_rgba(0,18,51,0.2)] p-0.5">
          {/* Ambient Glow */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-24 bg-gradient-to-b from-[#0070E0]/10 to-transparent blur-3xl pointer-events-none" />

          <div className="relative z-10 p-8 pt-9">
            {/* Header */}
            <div className="flex flex-col items-center text-center mb-7">
              <div className="w-14 h-14 rounded-[20px] bg-[#0070E0] shadow-xl shadow-[#0070E0]/30 flex items-center justify-center mb-6 border border-white/20 transform rotate-3">
              ...
              <p className="text-[14px] font-medium text-slate-500 leading-relaxed max-w-[280px]">
                Enjoy unlimited car diagnostics and professional reports with our Pro tier.
              </p>
            </div>
            ...
            <div className="space-y-3 mb-9 px-2">
              {benefits.map((benefit, idx) => (
                ...
                >
                  <div className="w-5.5 h-5.5 rounded-lg bg-emerald-50 border border-emerald-100 flex items-center justify-center shrink-0">
                    <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                  </div>
                  <span className="text-[14px] font-bold text-navy/80">{benefit}</span>
                </motion.div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              <motion.button
                whileHover={{ y: -2, boxShadow: "0 15px 30px rgba(0,112,224,0.3)" }}
                whileTap={{ scale: 0.98 }}
                onClick={handleWhatsAppActivate}
                className="w-full h-14 rounded-[20px] bg-gradient-to-br from-[#0070E0] via-[#005BB5] to-[#004A99] text-white font-black text-[12px] uppercase tracking-[0.15em] shadow-lg shadow-blue-600/15 transition-all flex items-center justify-center gap-2.5 group"
              >
