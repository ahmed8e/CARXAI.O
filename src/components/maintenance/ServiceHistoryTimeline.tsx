  ...
  if (records.length === 0) {
    return (
      <div className="bg-surface dark:bg-surface-high/30 rounded-[28px] border border-overlay p-8 text-center">
        ...
        <h3 className="text-base font-black text-on-surface mb-2">No Service Records Yet</h3>
        <p className="text-sm text-muted max-w-xs mx-auto">
          Log your first service to build your car's maintenance history — useful for resale value too.
        </p>
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="text-center">
            ...
            <p className="text-[9px] font-black uppercase tracking-widest text-muted">Records</p>
          </div>
          <div className="w-px h-8 bg-overlay" />
          <div className="text-center">
            <p className="text-xl font-black text-on-surface">${totalSpent.toLocaleString()}</p>
            <p className="text-[9px] font-black uppercase tracking-widest text-muted">Total Spent</p>
          </div>
        </div>
        <button
          onClick={onExport}
            ...
            >
              ...
              <div className="flex-1 bg-surface dark:bg-surface-high/30 rounded-[20px] border border-overlay p-4 mb-1">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h4 className="text-[13px] font-black text-on-surface tracking-tight">{record.serviceType}</h4>
                    {record.shopName && (
                  ...
                  {record.cost > 0 && (
                    <div className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400">
                      ...
                      <span className="text-sm font-black">{record.cost}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  ...
                  {record.mileage > 0 && (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[11px] font-bold text-muted">{record.mileage.toLocaleString()} mi</span>
                    </div>
                  )}
                </div>
