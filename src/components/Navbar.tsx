...
export default function Navbar({ onMenuClick, showNavLinks = false, transparent = false }: NavbarProps) {
  ...
  return (
    <nav className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-6xl flex items-center justify-between px-6 py-3 ${transparent ? 'bg-white/40' : 'bg-white/80'} backdrop-blur-xl border border-slate-200/60 rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.08)] transition-all duration-300`}>
      {/* Logo */}
